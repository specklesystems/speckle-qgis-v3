...
def setup():
    ...
    pythonExec = (
        get_qgis_python_path()
    )  # import specklepy; import os; print(os.path.abspath(specklepy.__file__)) #### import sysconfig; sysconfig.get_paths()['data'] # import qgis; import os; print(os.path.abspath(qgis.__file__)) # C:\Program Files\ArcGIS\Pro\Resources\ArcPy\arcpy\__init__.py
    ...
    try:
        ...
        if version("specklepy") == pkgVersion:
            pass
        else:
    ...
    try:
        ...
        subprocess_call([pythonExec, "-m", "pip", "install", f"{pkgName}"])
